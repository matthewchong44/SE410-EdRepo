...
<table>
    <thead><tr class="highlight"><th>Category Name</th><th>Description</th><th>Remove</th></tr></thead>
    {foreach $categories as $category}
        <tr><td><strong>{$category.name}</strong></td><td>{$category.description}</td>
        <td><a class="button" href="configureCategories.php?action=doRemove&categoryID={$category.ID}">Remove</a></td></tr>
    {/foreach}
</table>
