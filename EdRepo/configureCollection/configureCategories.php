      ...
      <?php
        if(!isset($userInformation)) {
          ...
          if($action=="display") {
            ...
            echo '<p>To add a new category, enter a title and a short description for the new category and select "Add".</p>';
            echo '<form name="addCategoryForm" action="configureCategories.php" method="post"><fieldset>';
            echo '<input type="hidden" readonly="readonly" name="action" value="doAdd"></input>';
            echo '<div class="fieldRow"><label for="categoryName"><strong>Category Name:</strong></label><input type="text" name="categoryName"></input></div>';
            echo '<div class="fieldRow"><label for="categoryDescription"><strong>Description:</strong></label><input type="text" name="categoryDescription"></input</div></fieldset>';
            echo '<fieldset class="buttons"><input type="submit" class="button" name="sub" value="Add"></input></fieldset>';
            echo '</form>';
            echo '</div>';
            echo '<p>Current categores in this collection:</p>';
            ...
            if($categories===FALSE || $categories=="NotImplimented" || count($categories)<=0) { //Make sure there wasn't an error getting categores and that at least one was found.
              echo '<p><span class="note">This collection currently contains no categories.</span><br>';
              echo 'To add a category, use the "Add Category" box at the top of this page.</p>';
            } else { //This else block runs if we got at least one category.
              echo '<table>';
			  echo '<tr class="highlight"><td>Category Name</td><td>Description</td><td>Remove</td></tr>';
            ...
            if($result===FALSE || $result==="NotImplimented") { //Error
              echo '<p class="alert negative"><img src="../lib/look/'.$LOOK_DIR.'/failure.png" alt="Failure"></img> Unabled to remove category.</p>';
            } else { //Successfully removed category
              echo '<p class="alert positive"><img src="../lib/look/'.$LOOK_DIR.'/success.png" alt="Success"></img> Category successfully removed.</p>';
            }
            ...
            if($result===FALSE || $result=="NotImplimented") { //Failed to create new category
              echo '<p class="alert negative"><img src="../lib/look/'.$LOOK_DIR.'/failure.png" alt="Failure"></img> Failed to create to category.</p>';
            } else { //Success
              echo '<p class="alert positive"><img src="../lib/look/'.$LOOK_DIR.'/success.png" alt="Success"></img> Category successfully created.</p>';
            }
            echo '<p><a href="configureCategories.php?action=display">Return to Category Configuration Panel</a></p>';
